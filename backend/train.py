...
def test(model, use_cuda, test_loader):
  ...
  with torch.no_grad():  # Tell the model that gradients need not be calculated
    for no, (data, target) in enumerate(tqdm(test_loader)):  # Get the batch
  ...
  test_loss /= total  # Accuracy = Total Correct / Total Samples
...
#
#   print('\ntestDataset: Average loss: {:.3f}, Accuracy: ({:.0f}%)\n'.format(
